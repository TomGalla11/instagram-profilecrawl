...
def csv_jos(filename, outfile=''):
    ...
    for i, p in enumerate(lol['posts']):
        hasil = {'caption': p['caption'], 'likes': p['likes']
                 ['count'], 'sumber': lol['username'], 'url': p['url']}
        posts.append(hasil)
...
def di_csv_kan():
    ...
    df.to_csv(
        f'kumpulan_{datetime.now().strftime("%d_%m_%Y")}_no_drop.csv', index=False)
    df.dropna(inplace=True)
    df.to_csv(
        f'kumpulan_{datetime.now().strftime("%d_%m_%Y")}.csv', index=False)
